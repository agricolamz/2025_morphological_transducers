...
Меня удивило, отсутствие в списках знака многоточия `…` (U+2026). Важно отметить, что при отсутствии раздела `DELIMITERS` в файле с правилами программа `cg3` выдает предупреждение. 

Кроме того, сущестует раздел `SOFT-DELIMITERS`, который я видел заполненный только запятой `"<.>"`.

Остальные списки задаются при помощи команды `LIST`, например,
...
- `SELECT` удаляет все чтения из когорты, кроме той, что подходит к контексту
...
- `REMOVE` удаляет чтение из когорты.
...
Напишите `cg3` правила, которые в нашем примере удаляет лишнее чтение и разметят синтаксические роли `S`, `O` и `V`. Избавьтесь также от предупреждения про разделители.
...
Мы посмотрели далеко не все возможности `cg3`: в документации можно найти инструкции по созданию новых когорт, добавлению зависимостей и отношений --- очень полезное для создание синтаксических корпусов, трибанков.

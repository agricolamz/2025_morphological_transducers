...
`vislcg3` --- это основная программа, которую мы будем использовать. Она по умолчанию угадывает формат, в котором ей поступают данные на вход и отдает в таком же формате. Однако есть и программы более узкого назначения:
...
В программе `vislcg3` достаточно много аргументов (см. `vislcg3 -h`), нам важно три:

- `-g` --- указывает файл с CG правилами;
- `-I` --- файл для чтения инпута вместо стандартного stdin;

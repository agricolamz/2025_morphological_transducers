...
```
КОМАНДА (тэги) (целевая группа) (опционально: некоторое условие)
```
...
Блокирующее поведение команд `MAP` и `REPLACE` можно снять при помощи команды `UNMAP`. По-умолчанию `UNMAP` работает только с единственными чтениями, но если хочется чтобы блокировка была снята с нескольких чтений одной формы, стоит добавить `UNSAFE`.

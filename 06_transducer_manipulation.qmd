...
Заменяет единицы в трансдьюсере:
...
```{{shell}}
$ hfst-substitute -i generator.hfst -f "ч" -t "@0@" | hfst-fst2strings
```

```{python}
!hfst-substitute -i examples/generator.hfst -f "ч" -t "@0@" | hfst-fst2strings
```
